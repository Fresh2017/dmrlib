...
bool init_repeater()
{
    ...
    if (config->sms_path != NULL) {
        config->sms = dmr_sms_new(config->sms_path, NULL, NULL);
        if (config->sms == NULL) {
            valid = false;
            dmr_log_critical("noisebridge: %s: failed to setup SMS at path %s",
                config->filename, config->sms_path);
        }
    }
...
static int loop_repeater_start_protos(void)
{
    ...
    if (config->sms != NULL) {
        dmr_log_info("noisebridge: starting sms proto");
        if ((ret = dmr_proto_call_init(config->sms)) != 0)
            return ret;
        if ((ret = dmr_proto_call_start(config->sms)) != 0)
            return ret;
    } else {
        dmr_log_info("noisebridge: sms not enabled");
    }
...
bool loop_repeater(void)
{
    ...
    if (config->mbe != NULL &&
        ...
        goto bail_loop_repeater;
    if (config->sms != NULL &&
        dmr_repeater_add(repeater, config->sms, &config->sms->proto) != 0)
        goto bail_loop_repeater;
