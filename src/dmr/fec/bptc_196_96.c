...
{
	return true; // TODO(pd0mz): implement
}

static void bptc_196_96_decode_data(dmr_bptc_196_96_t *bptc, uint8_t *data)
{
	bool bits[96];
	uint8_t i, j = 0;
	for (i = 4; i < 12; i++, j++)
...
int dmr_bptc_196_96_decode(dmr_bptc_196_96_t *bptc, dmr_packet_t *packet, uint8_t *data)
{
	...
	if (!bptc_196_96_decode_parity(bptc)) {
		dmr_log_error("parity check failed");
...
static void bptc_196_96_encode_data(dmr_bptc_196_96_t *bptc, uint8_t *data)
{
	bool bits[96];
	uint8_t i, j = 0;
...
static void bptc_196_96_encode_parity(dmr_bptc_196_96_t *bptc)
{
	...
	for (row = 0; row < 9; row++) {
		pos = (row * 15) + 1;
		dmr_hamming_15_11_3_encode_bits(bptc->deinterleaved_bits + pos);
	}
}
