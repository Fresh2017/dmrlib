#include <string.h>

#include "dmr/fec/rs_12_9.h"

typedef struct {
	uint8_t error_locations[256];
	uint8_t errors_num;
} dmr_rs_12_9_roots_t;

static uint8_t rs_12_9_poly[] = {64, 56, 14, 1, 0, 0, 0, 0, 0, 0, 0, 0};

// See DMR AI. spec. page 138.
static uint8_t rs_12_9_exp[] = {
	
};

// See DMR AI. spec. page 138.
static uint8_t rs_12_9_log[] = {
	
};

static uint8_t rs_12_9_gmult(uint8_t a, uint8_t b)
{
	if (a == 0 || b == 0)
		return 0;

	return rs_12_9_exp[rs_12_9_log[a] + rs_12_9_log[b]];
}

void dmr_rs_12_9_encode(uint8_t data[DMR_RS_12_9_DATASIZE], uint8_t parity[DMR_RS_12_9_CHECKSUMSIZE])
{
	memset(parity, 0, DMR_RS_12_9_CHECKSUMSIZE);

	uint8_t i, j, k;
	for (i = 0; i < DMR_RS_12_9_DATASIZE; i++) {
		j = data[i] ^ parity[DMR_RS_12_9_CHECKSUMSIZE - 1];
		for (k = DMR_RS_12_9_CHECKSUMSIZE - 1; k > 0; k--) {
			parity[k] = parity[k - 1] ^ rs_12_9_gmult(rs_12_9_poly[k], j);
		}
		parity[0] = rs_12_9_gmult(rs_12_9_poly[0], j);
	}
}

bool dmr_rs_12_9_check(uint8_t data[DMR_RS_12_9_DATASIZE + DMR_RS_12_9_CHECKSUMSIZE])
{
	uint8_t parity[DMR_RS_12_9_CHECKSUMSIZE];
	dmr_rs_12_9_encode(data, parity);
	return !memcmp(data + DMR_RS_12_9_DATASIZE, parity, DMR_RS_12_9_CHECKSUMSIZE);
}