...
DMR_PRV const uint8_t POLY[] = {64, 56, 14, 1, 0, 0, 0, 0, 0, 0, 0, 0};

DMR_PRV const uint8_t EXP_TABLE[] = {
...
DMR_PRV const uint8_t LOG_TABLE[] = {
...
DMR_PRV static uint8_t gmult(uint8_t a, uint8_t b)
{
  if (a == 0U || b == 0U)
	  return 0U;

  uint8_t i = LOG_TABLE[a];
  uint8_t j = LOG_TABLE[b];

  return EXP_TABLE[i + j];
}

DMR_PRV static void encode(uint8_t bytes[9], uint8_t parity[NPAR])
{
    // Clear parity
    memset(parity, 0, 3);
    ...
    int8_t j;
    for (i = 0; i < 9; i++) {
        uint8_t dbyte = bytes[i] ^ parity[NPAR - 1];
        for (j = NPAR - 1; j > 0; j--) {
...
DMR_API int dmr_rs_12_9_4_decode(uint8_t bytes[12])
{
    if (bytes == NULL)
...
DMR_API int dmr_rs_12_9_4_encode(uint8_t bytes[12])
{
    if (bytes == NULL)
        return dmr_error(DMR_EINVAL);
