...
char *dmr_dpf_name(dmr_dpf_t dpf)
{
	uint8_t i;
...
char *dmr_sap_name(dmr_sap_t sap)
{
	uint8_t i;
...
int dmr_data_header_decode(dmr_data_header_t *header, dmr_packet_t *packet)
{
	if (packet == NULL || header == NULL || packet->data_type >= DMR_DATA_TYPE_INVALID)
...
int dmr_data_block_decode(dmr_data_block_t *block, bool confirmed, dmr_packet_t *packet)
{
	...
	uint16_t crc = 0;
	uint8_t i;
	memset(block, 0, sizeof(dmr_data_block_t));
	if (packet->data_type == DMR_DATA_TYPE_RATE34_DATA) {
		block->length = dmr_data_block_size(DMR_DATA_RATE34, confirmed);

		uint8_t bytes[block->length + 2];
		memset(bytes, 0, sizeof(bytes));
		if (dmr_trellis_rate_34_decode(bytes, packet) != 0)
			return dmr_error(DMR_LASTERROR);
...
uint8_t dmr_data_block_size(dmr_data_rate_t rate, bool confirmed)
{
	switch (rate) {
